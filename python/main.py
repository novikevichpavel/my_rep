...
#         if cls.__is_connected == None:
#             cls.__is_connected = super().__new__(cls)

#         return cls.__is_connected
    

#     def __del__(self):
#         DataBase.__is_connected = None


#     def __init__(self, user_name, user_password, port):
...
#     def connect(self):
#         print(f"Connected succesfully {self.user_name}, {self.user_password}, {self.port}")
...
#     @classmethod
#     def user_input(cls):
#         x = int(input("First arg: "))
#         y = int(input("Second arg: ")) 
#         return cls(x, y)
...
#             self.x = x
#             self.y = y
#             print(f"Сумма квадратных чисел: {self.norm(self.x, self.y)}")
...
class Point:
    ...
    @classmethod
    def __check_value_type(cls, x):
        return type(x) in (int, float)


    def set_nums(self, x, y):
        if self.__check_value_type(x) in (int, float) and self.__check_value_type(x) in (int, float):
            self.__x = x
            self.__y = y
        else:
            raise ValueError("The args must be of integer type!")


    @classmethod    
    def user_input(cls):
        x = int(input("Enter once: "))
        y = int(input("Enter twice: "))
        return cls(x, y)
...
point_one = Point.user_input()

print(point_one.get_nums())
